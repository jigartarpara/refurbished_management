...
frappe.ui.form.on("Refurb Order", {
	refresh(frm) {
        if(!cur_frm.doc.__onload.device_transfer_entry && !cur_frm.doc.__onload.manufacturing){
        ...
        if(!cur_frm.doc.__onload.manufacturing){
            frm.add_custom_button(
            ...
            frm.add_custom_button(
                ...
                function () {
                    if(cur_frm.doc.__onload.draft_manufacturing){
                        frappe.set_route("Form", "Stock Entry", cur_frm.doc.__onload.draft_manufacturing);
                    }else{
                        frappe.model.open_mapped_doc({
